...
public:
    ...
    static vector<Vertex> generateCircleFan(const glm::vec3& center, float radius, int segments = 36, bool useColor = false, glm::vec4 color = glm::vec4(1.0f)) {
        ...
        for (int i = 0; i <= segments; ++i) {
            ...
            vertices.push_back({ pos, uv, color, normal });
        }
        return vertices;
    }
    ...
    static vector<Vertex> generateCylinderSide(const glm::vec3& center, float radius, float height, int segments = 36, bool cone = false, glm::vec4 color = glm::vec4(1.0f)) {
        ...
        for (int i = 0; i <= segments; i++) {
            float theta = i * angleStep;
            glm::vec3 dir = glm::normalize(glm::vec3(glm::cos(theta), glm::sin(theta), 0));
            ...
            vertices.push_back({ bottomPos, bottomUV, color, dir });
            vertices.push_back({ topPos, topUV, color, dir });
        }
        return vertices;
    }
    ...
    static Mesh generatePyramid(const glm::vec3& center, float baseSize, float height, bool useColor = false, glm::vec4 color = glm::vec4(1.0f)) {
        ...
        // Base
        addTri({ center.x - h, center.y - h, center.z }, { center.x + h, center.y - h, center.z }, { center.x + h, center.y + h, center.z }, { 0,0 }, { 1,0 }, { 1,1 });
        addTri({ center.x - h, center.y - h, center.z }, { center.x + h, center.y + h, center.z }, { center.x - h, center.y + h, center.z }, { 0,0 }, { 1,1 }, { 0,1 });
        // Sides
        addTri({ center.x - h, center.y - h, center.z }, { center.x + h, center.y - h, center.z }, top, { 0,0 }, { 1,0 }, { 0.5,1 });
        addTri({ center.x + h, center.y - h, center.z }, { center.x + h, center.y + h, center.z }, top, { 0,0 }, { 1,0 }, { 0.5,1 });
        addTri({ center.x + h, center.y + h, center.z }, { center.x - h, center.y + h, center.z }, top, { 0,0 }, { 1,0 }, { 0.5,1 });
        addTri({ center.x - h, center.y + h, center.z }, { center.x - h, center.y - h, center.z }, top, { 0,0 }, { 1,0 }, { 0.5,1 });
    ...
    static Mesh generatePrism(glm::vec3 center, float radius, float height, int sides, bool useColor = false, glm::vec4 color = glm::vec4(1.0f)) {
        ...
        for (int i = 0; i < sides; i++) {
            ...
            glm::vec3 normal = glm::normalize(glm::cross(t2p - b1, t1p - b1));
