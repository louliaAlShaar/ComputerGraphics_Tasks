...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // äÝÚøá ÇÓÊÎÏÇã ÇáÜ texture
        shader.setBool("useTexture", true);       // Îáí ÇáÔíÏÑ íÓÊÎÏã texture
        shader.setBool("useVertexColor", false);  // äÎáí áæä vertices ãÇ íÃËÑ
        // flatColor ããßä ÊÊÑßå Ãæ íßæä Ãí ÞíãÉ¡ ãÇ ÑÇÍ íÃËÑ ØÇáãÇ useTexture=true

        glActiveTexture(GL_TEXTURE0);             // ÊÝÚíá æÍÏÉ ÇáÜ texture 0
        glBindTexture(GL_TEXTURE_2D, texture);   // ÑÈØ ÇáÜ texture Çááí ÚäÏß
        shader.setInt("tex", 0);                  // ÖÈØ sampler Ýí ÇáÔíÏÑ Úáì ÇáæÍÏÉ 0
